  ...
  return (
    <article className={`highlight-card ${critical ? 'critical' : elevated ? 'elevated' : ''}`}>
      ...
      <div className="card-actions">
        ...
        <button type="button" className={item.highlighted ? 'feedback-active highlighted' : ''} aria-pressed={item.highlighted} disabled={Boolean(feedbackBusy)} title={item.highlighted ? 'Remove highlight' : 'Mark as highlighted'} onClick={() => feedback(item.highlighted ? 'unhighlight' : 'highlight')}>{feedbackBusy === 'highlight' || feedbackBusy === 'unhighlight' ? 'Saving…' : item.highlighted ? 'Highlighted' : 'Highlight'}</button>
